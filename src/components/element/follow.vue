  ...
  <div class="fy">
    <ul class="page">
      ...
      <li v-for="page in followList.pages" :key="page">
        <a :href="`/friends/follow?page=` + page + `&size=8&id=` + $route.query.id">
          {{ page }}
        </a>
      </li>
      &nbsp;
...
<script setup>
import { getFollowByIdApi } from "/src/utils/api.js";
import { useRouter, useRoute } from "vue-router";
import { reactive, ref } from "@vue/reactivity";
const $router = useRouter();
const $route = useRoute();
let followList = ref();
followList = (await getFollowByIdApi($route.query.id)).data.resultData;
let urlPrefix = ref(`/friends/follow?page=1` + `&size=5&id=` + $route.query.id);
let urlSuffix = ref(
  `/friends/follow?page=` + followList.pages + `&size=5&id=` + $route.query.id
);
</script>
