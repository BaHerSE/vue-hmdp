<template>
  ...
  <div class="star">
    <span>给个评分吧：</span>
    <t-rate show-text size="18" v-model="sValue"></t-rate>
  </div>
  <div class="pl">
    <textarea
      name=""
  ...
  <div class="up">
    <button class="submit" @click="cinecism">发送</button>
  </div>
  <img src="" alt="" srcset="" />
</template>

<script setup>
import { getMovieIdApi, getScoreByIdApi } from "/src/utils/api.js";
...
function cinecism() {
  ...
  axios.post("/Blog/writeCinecism", cinecism).then((result) => {
    $router.push({ path: "/list", query: { Mid: Mid } });
  });
}
</script>
